...
import { EnumProductsSort, getAllProductsDto } from './dto/getAll-products.dto'
...
export class ProductService {
	...
	async getSimilar(id: number) {
		const currentProduct = this.byId(id)
		...
		const products = this.prisma.product.findMany({
			where: {
				category: {
					name: (await currentProduct).category.name
				},
				NOT: {
					id: (await currentProduct).id
				}
			},
			orderBy: {
				createdAt: 'desc'
